    ...
    with torch.no_grad():  #turn of gradient decent
        for batch, (data, target) in enumerate(test_loader):
            ...
            if cutoff != -1: # set to -1 for not generating the roc curve
                ...
                total_number_correct += predicted_values.eq(target.data.view_as(predicted_values)).sum() # .eq compares pred and target and gives true for a match,
                                                                                                          # and false for a miss. .sum() adds num of True, view_as is for the dimentional matc
                for i in range(len(output)):
